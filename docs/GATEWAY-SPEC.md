...
- ProjectX read circuit breaker: degrade explicitly, isolated per endpoint family. It is not the REST in-flight cap (`RestConcurrencyGate` / `/health.rest_concurrency`). Merging the unification into shadow/PRAC requires only a 2h clean interim run (zero unexplained restarts/duplicates); trusting it for continuous **armed** operation still requires the full 72h PRAC soak (`TS-STREAM-RECOVERY-01`). New exposure stays blocked while `state_complete=false` via `buildExecutionGates` / `validateEntryRisk`.
