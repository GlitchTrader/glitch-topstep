...
**Pending live validation (PR [#38](https://github.com/GlitchTrader/glitch-topstep/pull/38)):** the ownership projection had accumulated 52 phantom tranches while the account was flat, so the packet reported the protection of the oldest phantom and entries were rejected with `protection_not_proven` while their brackets were working on the venue. Ownership now derives from the contract count the venue reports open. Two venue behaviours are recorded here because they contradict the spec ideal and the fixes depend on them: Auto OCO **cancels the whole bracket group** when a partial exit fills, leaving the surviving tranche with open contracts and zero working orders, so the dropped legs are re-placed; and `MOVE_STOP` edits the working order in place, so a re-placed leg must use the price the venue last held rather than the entry payload, or a tightened stop silently widens.

Scenario A is proven live on PRAC (03:27:11–03:30:39 UTC, PRAC-V2-645601-65050593): all seven checks pass, including `proven_a` — the original fatal — and `tranche_a_alive`. That run's `fatal: position_flat_before_live_protection` is **not** gateway behaviour: a second E2E process started inside its window and its `ensureFlat` closed the position under observation. **Scenarios B (restart) and C (LONG mirror) remain unvalidated** and are the open item for PM-4 Phase C acceptance. Live repo and gateway are aligned on `eb1c026` with a `dist/` built from that commit. Two cautions when picking that up: run only one E2E process at a time, and expect ProjectX velocity control to absorb the first attempts if the account has traded recently — a stalled run is usually a 3-minute cooldown, not a hang.
