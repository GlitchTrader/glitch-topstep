...
describe("position management coordinator", () => {
  ...
  it("rearms protective orders for surviving tranches when live brackets are missing", async () => {
    ...
    try {
      ...
      const entryIntent: TradeIntent = {
        ...
        decisionAudit: {
          ...
          bearCase: "Bear case.",
          flatCase: "Flat case.",
          aggressiveCase: "Aggressive case.",
          conservativeCase: "Conservative case.",
          decisiveEvidence: "Evidence.",
          disconfirmingEvidence: "Counter evidence.",
          changeCondition: "Change condition.",
          finalChoice: "ENTER_SHORT",
        },
        quantity: 1,
        orderType: "MARKET",
        stopLoss: 19_990,
      ...
      store.registerIntent(entryIntent, now.toISOString());
      const placed: Array<{ customTag?: string | null; type: number }> = [];
      const api = {
        placeOrder: async (request: { customTag?: string | null; type: number }) => {
          placed.push(request);
      ...
      const changed = await coordinator.rearmTrancheProtection(current);
      assert.equal(changed, true);
      assert.equal(placed.length, 2);
      assert.equal(placed[0]?.customTag, `glt-${ENTRY_INTENT_ID}-SL`);
      assert.equal(placed[1]?.customTag, `glt-${ENTRY_INTENT_ID}-TP`);
    } finally {
      store.close();
      rmSync(directory, { recursive: true, force: true });
    }
