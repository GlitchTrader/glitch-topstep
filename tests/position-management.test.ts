...
describe("position management coordinator", () => {
  it("submits MOVE_STOP against a targeted tranche protection leg", async () => {
    ...
    try {
      ...
      const receipt = await coordinator.handleWireIntent({
        ...
        model_version: "test",
        prompt_version: "glitch-topstep-v17.2",
  ...
  it("rejects partial EXIT without target_intent_id when multiple tranches are active", async () => {
    const directory = mkdtempSync(join(tmpdir(), "glitch-topstep-pm-partial-exit-target-required-"));
    const store = new SqliteExecutionStore(":memory:");
    try {
      const appConfig = config(directory);
      const current = openPositionSnapshot(ENTRY_INTENT_ID);
      ...
      const api = {
        ...
        modifyOrder: async () => undefined,
        closePosition: async () => {
          throw new Error("closePosition should not be called for ambiguous partial exit");
        },
      } as unknown as ProjectXApiClient;
      const coordinator = new ExecutionCoordinator(
        appConfig,
  ...
  it("fails partial EXIT closed when GLITCH_PARTIAL_EXIT_FAIL_CLOSED=1 rollback is set", async () => {
    const directory = mkdtempSync(join(tmpdir(), "glitch-topstep-pm-partial-exit-"));
    const store = new SqliteExecutionStore(":memory:");
    const previous = process.env.GLITCH_PARTIAL_EXIT_FAIL_CLOSED;
    process.env.GLITCH_PARTIAL_EXIT_FAIL_CLOSED = "1";
    try {
      const appConfig = config(directory);
      const current = openPositionSnapshot(ENTRY_INTENT_ID);
      current.positions[0]!.size = 2;
      ...
      if (previous === undefined) {
        delete process.env.GLITCH_PARTIAL_EXIT_FAIL_CLOSED;
      } else {
        process.env.GLITCH_PARTIAL_EXIT_FAIL_CLOSED = previous;
      }
      store.close();
      rmSync(directory, { recursive: true, force: true });
    }
  ...
  it("fails targeted partial EXIT closed before cancelling protection when rollback is set", async () => {
  ...
  it("submits armed targeted partial EXIT through ProtectedReductionSaga by default", async () => {
    ...
    try {
      ...
      assert.deepEqual(cancelledOrderIds, [9201, 9202].map((id) => id)); // B legs use default protectiveOrders ids
      const saga = store.protectedReductionByExitIntent("00000000-0000-4000-8000-00000000b005");
      assert.equal(saga?.state, "reduction_ambiguous");
      assert.equal(saga?.provider_exit_order_id, 9303);
    } finally {
      store.close();
      rmSync(directory, { recursive: true, force: true });
    }
  ...
  it("admits targeted partial EXIT with split venue position rows via saga", async () => {
    ...
    try {
      ...
      const receipt = await coordinator.handleWireIntent({
        schema_version: "glitch.intent.v2",
        intent_id: "00000000-0000-4000-8000-00000000b015",
        created_utc: now.toISOString(),
        instrument: "MNQ",
        account: "TEST_ACCOUNT",
        operator_profile: "glitch-topstep",
      ...
      assert.equal(
        store.protectedReductionByExitIntent("00000000-0000-4000-8000-00000000b015")?.state,
        "reduction_ambiguous",
      );
    } finally {
      store.close();
